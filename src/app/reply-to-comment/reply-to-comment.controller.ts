...
class ReplyToCommentRouteControllerClass {
    ...
    /** Vote for the comment. Accepts "like" or "dislike" */
    delete_like = async (req: ReplyForCommentSectionRequestTypes["delete_like"], res: ExpressJS.Response) => {
        const { dto, sessionDto } = checkRequestForValidity(req, ["dto", "sessionDto"]);
    ...
    delete_dislike = async (req: ReplyForCommentSectionRequestTypes["delete_dislike"], res: ExpressJS.Response) => {
        const { dto, sessionDto } = checkRequestForValidity(req, ["dto", "sessionDto"]);
