...
import type {
    optionalMessage,
    optionalMessageAndData,
    optionalMessageAndErrors,
...
export const goReplyHttp = new (class goReplyHttpServiceClass {
    ...
    created = <T>(res: ExpressJS.Response, { message = "Created", data }: optionalMessageAndData<T>) => {
        return allHttpResponseHandler({ res, response_code: responseHTTPCodes.CREATED, message, data, ok: true });
    };
    accepted = <T>(res: ExpressJS.Response, { data, message = "Accepted" }: optionalMessageAndData<T>) => {
        return allHttpResponseHandler({ res, response_code: responseHTTPCodes.ACCEPTED, message, data, ok: true });
    };
    bad_request = (res: ExpressJS.Response, { errors, message = "Bad Request Error" }: optionalMessageAndErrors) => {
